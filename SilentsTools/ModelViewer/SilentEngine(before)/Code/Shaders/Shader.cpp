...
D3D12_INPUT_LAYOUT_DESC CIlluminatedTexturedShader::CreateInputLayout()
{
	...
	pd3dInputElementDescs[3] = { "BONEINDEX", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 24, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[4] = { "WEIGHT", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 24, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
...
void CObjectsShader::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	for (int index = 0; index < m_nObjects; ++index)
	{
		CB_OBJECT_INFO* pbMappedcbGameObject = (CB_OBJECT_INFO *)((UINT8 *)m_pcbMappedGameObjects + (index * ncbElementBytes));
		XMStoreFloat4x4(&pbMappedcbGameObject->m_xmf4x4World, XMMatrixTranspose(XMLoadFloat4x4(&m_ppObjects[index]->m_xmf4x4World)));

		::memcpy(pbMappedcbGameObject->m_bone, m_ppObjects[index]->GetBoneData(), sizeof(XMFLOAT4X4) * m_ppObjects[index]->GetBoneNum());
...
void CObjectsShader::SetRootParameter(ID3D12GraphicsCommandList *pd3dCommandList)
{
