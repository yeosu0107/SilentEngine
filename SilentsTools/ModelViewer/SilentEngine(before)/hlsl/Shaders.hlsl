...
//{
//	float4x4 gBoneTransforms[96] : packoffset(c0);
//}
...
struct VS_TEXTURED_LIGHTING_INPUT
{
	...
	uint4	 index : BONEINDEX;   // 뼈 인덱스
	float4	 weight : WEIGHT;       // 가중치
};
...
VS_TEXTURED_LIGHTING_OUTPUT VSTexturedLighting(VS_TEXTURED_LIGHTING_INPUT input)
{
	...
	//posL += (float3)weights[0] * mul(float4(input.position, 1.0f), gBoneTransforms[4]);
